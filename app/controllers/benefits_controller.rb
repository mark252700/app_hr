class BenefitsController < ApplicationController
  ...
  def new
    ...
    # Find the EmpDetail associated with the current user
    @emp_detail = EmpDetail.find_by(user_id: @user_id)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @benefit.save
        format.html { render 'emp_details/new' }
        format.json { render :show, status: :created, location: @benefit }
        format.turbo_stream { render turbo_stream: turbo_frame_tag("benefits_table") { render "benefits_table" } }
      else
